...
void main(){
	...
	texcoord =  in_texcoord1 ;
		...
		in_position5 = mix( in_position1 , in_position3,happy_w) ;
 
		pos_eye = (view_matrix * model_matrix * vec4(in_position5,1.0)).xyz;
	 
	
	Lp_eye = (view_matrix * vec4(1.0,1.0,10.0,1.0)).xyz;
	 
 
		gl_Position = projection_matrix*view_matrix*model_matrix*vec4(in_position5,1); 
